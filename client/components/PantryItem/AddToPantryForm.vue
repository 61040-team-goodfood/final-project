  ...
  watch: {
    pantryItem: function(newItem, oldItem) {
      this.fields = [
        ...
        { type: 'reminder', id: 'remindDays', label: 'Remind Me', value: newItem.expirationDate ? Math.ceil((new Date(newItem.expirationDate) - new Date(reminder.date))/ (1000 * 60 * 60 * 24)) : 3, placeholder: 'Enter number...', tooltip:'Reminders can only be set to a future date' }
      ];
    }
  },
  data() {
